...
const Icons = {
    ...
    Projects: (props) => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>,
    ...
    Send: (props) => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><line x1="22" x2="11" y1="2" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>,
...
const signals = [
    { id: 's1', type: 'AI News', date: '2h ago', title: 'OpenAI DevDay Keynote', content: 'サム・アルトマンの発言から見る、次世代のエージェント指向。自律型AIが組織運営に与える影響について分析しました。', tags: ['OpenAI', 'Analysis'] },
    { id: 's2', type: 'Log', date: '5h ago', title: 'みんなの事務局 PoC開始', content: '初期導入3団体において、経理業務のAI自動化率の測定を開始しました。想定より高い精度が出ています。', tags: ['Experiment', 'Data'] },
    { id: 's3', type: 'AI News', date: '1d ago', title: 'Anthropicの"Constitutional AI"', content: '倫理憲章に基づくAIの意思決定プロセスは、NPOのガバナンスに応用できるか？技術文書の解読メモ。', tags: ['Ethics', 'Research'] },
];

const discussions = [
    { id: 'd1', user: 'Daiki.T', role: 'Researcher', text: '「資本主義外」とありますが、持続可能性の担保としてトークンエコノミーの導入は検討していますか？', time: '10m ago', likes: 5, replies: 1 },
    { id: 'd2', user: 'nexs_admin', role: 'Owner', text: 'ありがとうございます。はい、地域通貨プロジェクトでの知見を活かし、貢献度に応じた非金銭的インセンティブ（評判スコア等）の実装を計画中です。', time: '5m ago', likes: 2, replies: 0 },
    { id: 'd3', user: 'Guest_User', role: 'Visitor', text: '事務局のアウトソーシングはセキュリティ面での懸念を持つ団体も多そうです。', time: '1h ago', likes: 8, replies: 0 },
];
                ...
                {tabs.map((tab) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <span className={`text-[10px] font-bold tracking-tight ${isActive ? 'opacity-100' : 'opacity-0'}`}>
                                {tab.label}
                            </span>
                        </button>
                    );
    ...
    <div className="bg-gradient-to-br from-white to-gray-50 p-6 rounded-xl shadow-soft border border-gray-200 relative overflow-hidden group transition-all animate-fade-in-up mb-8">
        <div className="relative z-10">
            <div className="flex justify-between items-start mb-2">
                <p className="font-mono text-xs text-gray-400 uppercase tracking-widest">About Us</p>
                <button
...
const ProjectCard = ({ project, onClick }) => (
    ...
    >
        <div className="flex justify-between items-start mb-3">
            <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${project.color} ${project.textColor}`}>
                {project.category}
            </span>
            <div className={`w-2 h-2 rounded-full ${project.status === 'In Progress' ? 'bg-green-500 animate-pulse' : 'bg-gray-300'}`}></div>
        </div>
        <h3 className="text-lg font-bold text-gray-900 mb-2 leading-tight group-hover:text-gray-600 transition-colors">{project.title}</h3>
...
const SignalCard = ({ signal }) => (
    <div className="flex gap-4 p-4 border-b border-gray-50 last:border-0 bg-white active:bg-gray-50 transition-colors cursor-pointer">
        <div className="shrink-0 pt-1">
             <div className={`w-10 h-10 rounded-full flex items-center justify-center ${signal.type === 'AI News' ? 'bg-black text-white' : 'bg-gray-100 text-gray-500'}`}>
                <Icons.Zap size={18} />
             </div>
        </div>
        <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
                <span className="text-[10px] font-bold text-gray-500">{signal.type}</span>
                <span className="text-[10px] text-gray-300">•</span>
                <span className="text-[10px] text-gray-400 font-mono">{signal.date}</span>
            </div>
            <h4 className="text-sm font-bold text-gray-900 mb-1 leading-snug">{signal.title}</h4>
            <p className="text-xs text-gray-600 leading-relaxed line-clamp-2 mb-2">
                {signal.content}
            </p>
            <div className="flex flex-wrap gap-1">
                {signal.tags.map(tag => (
                    <span key={tag} className="text-[9px] px-1.5 py-0.5 bg-gray-50 border border-gray-100 rounded text-gray-500">#{tag}</span>
                ))}
            </div>
        </div>
    </div>
        ...
        <>
            ...
            >
                ...
                <div className="px-5 pb-4 border-b border-gray-100">
                    <div className="flex items-center gap-2 mb-1">
                         <span className={`w-2 h-2 rounded-full ${project?.status === 'In Progress' ? 'bg-green-500' : 'bg-gray-300'}`}></span>
                         <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{project?.category}</span>
                    </div>
                    <h3 className="text-base font-bold text-gray-900 line-clamp-1">{project?.title}</h3>
                </div>
                        ...
                        <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl text-xs text-blue-900 flex gap-3">
                            ...
                            <div>
                                ...
                                <p className="leading-relaxed opacity-80">
                                    この議論は公開され、誰でも閲覧可能です。建設的な対話を心がけてください。<br/>
                                    <span className="opacity-60 text-[10px] mt-1 block">License: CC BY 4.0</span>
                                </p>
                            </div>
                        </div>
                            ...
                            <div key={item.id} className="flex gap-3">
                                ...
                                <div className="flex-1">
                                    <div className="flex items-baseline justify-between mb-1">
                                        <span className="text-xs font-bold text-gray-900">{item.user} <span className="text-[9px] font-normal text-gray-500 px-1.5 py-0.5 bg-gray-100 rounded-full ml-1">{item.role}</span></span>
                ...
                <div className="absolute bottom-0 inset-x-0 bg-white p-4 border-t border-gray-100 pb-safe z-10">
                    ...
                    <p className="text-[10px] text-gray-400 text-center mt-2">
                        投稿には <span className="underline cursor-pointer hover:text-gray-600">ログイン (Auth)</span> が必要です
                    </p>
                </div>
            </div>
        </>
...
const HomeView = ({ setActiveTab, openProject }) => {
    ...
    return (
        <div className="pb-32 pt-16 animate-fade-in-up px-5">
            {showCard && (
                <ConceptCard onClose={() => setShowCard(false)} onNavigate={() => setActiveTab('about')} />
            )}
            ...
            <section className="mb-8">
                <div className="flex justify-between items-center mb-3">
                    ...
                    <button onClick={() => setActiveTab('signals')} className="text-xs text-gray-500 font-medium hover:text-black">More</button>
                </div>
            ...
            <section>
                 <div className="flex justify-between items-center mb-3">
                    <h2 className="text-sm font-bold text-gray-900 flex items-center gap-2">
                        <Icons.Projects size={16} className="text-blue-500" /> Active Experiments
                    </h2>
                    <button onClick={() => setActiveTab('projects')} className="text-xs text-gray-500 font-medium hover:text-black">View All</button>
                </div>
...
const ProjectsView = ({ openProject }) => (
    <div className="pb-32 pt-16 animate-fade-in-up px-5 min-h-screen">
        ...
        <p className="text-xs text-gray-500 mb-6">進行中の社会実装プロジェクトと検証項目。</p>
...
const SignalsView = () => (
    <div className="pb-32 pt-16 animate-fade-in-up min-h-screen">
        <div className="px-5 mb-4">
            ...
            <p className="text-xs text-gray-500">AIが収集した業界動向と、研究員による実験ログ。</p>
        </div>
        ...
        <div className="bg-white border-t border-gray-100">
             ...
             {signals.map(signal => (
                <SignalCard key={signal.id + '_dup'} signal={{...signal, id: signal.id + '_dup', date: '2d ago'}} />
            ))}
        </div>
    </div>
);
...
const AboutView = () => (
    <div className="pb-32 pt-16 px-5 min-h-screen bg-white animate-fade-in-up">
        <div className="mb-8">
            <p className="font-mono text-xs text-gray-400 uppercase tracking-widest mb-2">Organization</p>
        ...
        <div className="space-y-6">
            ...
            <dl className="space-y-4 text-sm border-t border-gray-100 pt-4">
                <div className="grid grid-cols-3 gap-4">
                    <dt className="text-gray-500 text-xs font-bold uppercase pt-1">Name</dt>
                    <dd className="col-span-2 font-medium text-gray-800">次世代社会デザイン研究機構</dd>
                </div>
                <div className="grid grid-cols-3 gap-4">
                    <dt className="text-gray-500 text-xs font-bold uppercase pt-1">Status</dt>
                    <dd className="col-span-2 font-medium text-gray-800">NPO法人化準備中</dd>
                </div>
            </dl>

            <div className="pt-6 border-t border-gray-100">
                 <a href="mailto:info@nexs.jp" className="flex items-center justify-between p-4 bg-black text-white rounded-xl shadow-lg active:scale-[0.98] transition-transform">
    ...
    return (
        <div className="fixed inset-0 z-[60] bg-white flex flex-col animate-fade-in-up">
            ...
            <div className="flex-1 overflow-y-auto px-5 pb-32">
                <div className="pt-6">
                    <span className={`inline-block px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider mb-4 ${project.color} ${project.textColor}`}>
                        {project.category}
                    </span>
                            ...
                            {project.hypotheses.map((item, idx) => (
                                <div key={idx} className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                                    <div className="flex items-start gap-3">
                                        ...
                                        <p className="text-sm font-medium text-gray-800 leading-snug">{item}</p>
                                    </div>
                                    <div className="mt-3 pl-8 flex gap-2">
                                        <span className="text-[10px] bg-white px-2 py-0.5 rounded border border-gray-100 text-gray-500">データ収集中</span>
                                        <button
                    ...
                    {/* Outputs / Reports */}
                    <div className="mb-12">
                         <h3 className="text-xs font-bold text-gray-900 uppercase tracking-wider mb-4">Outputs</h3>
            ...
            <div className="fixed bottom-0 inset-x-0 bg-white border-t border-gray-100 p-4 pb-safe z-[70]">
                <div className="max-w-md mx-auto flex gap-3">
                    ...
                    >
                        <Icons.MessageSquare size={18} /> 議論に参加 ({project.discussionCount})
                    </button>
                </div>
            </div>
        </div>
...
const App = () => {
    ...
    const renderContent = () => {
        switch (activeTab) {
            case 'home': return <HomeView setActiveTab={setActiveTab} openProject={setSelectedProject} />;
            case 'projects': return <ProjectsView openProject={setSelectedProject} />;
            case 'signals': return <SignalsView />;
            case 'about': return <AboutView />;
            default: return <HomeView setActiveTab={setActiveTab} openProject={setSelectedProject} />;
        }
    };
    ...
    return (
        <div className="bg-gray-200 min-h-screen font-sans text-gray-900 flex justify-center">
            <div className="w-full max-w-md bg-white min-h-screen shadow-2xl relative overflow-hidden flex flex-col">
                <Header title={activeTab === 'about' ? 'About' : 'nexs'} showSearch={activeTab !== 'about'} />
