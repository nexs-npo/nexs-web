...
#### Step 4: ファイル冒頭の【変更内容】を記録する

編集後、ファイルの冒頭にあるコメント内の **【変更内容】** セクションに、何を変更したかを記録してください。

```html
<!-- design-components/about.html の冒頭部分 -->
  【変更内容】
  スタッフが編集するときは、ここに何を変更したかを記録してください。
  例：
    - 2026-01-21: Hero Section の説明文を修正
    - 2026-01-20: "Our Values" のセクションを追加
```

**記録例：**
```
【変更内容】
  - 2026-01-22: Hero Section の H1 テキストを「社会課題に、科学する。」に更新
  - 2026-01-22: Process Cycle のステップ説明を簡潔に修正
```

このコメントを記録しておくことで、開発AIが「どこが変わったか」を素早く把握できます。

#### Step 5: スクリーンショットと一緒に報告
...
1. **【変更内容】を確認**
   ```
   コンポーネントファイルの冒頭コメント内の【変更内容】セクションを読んで、
   何が更新されたのかを把握する。
   ```
